...
export function Signpost({
  ...
  const rots: number[] = [];
  planks.forEach((p, i) => {
    const mag = i === 0 ? 5 + rand() * 2 : 6.5 + rand() * 4.5;
    const s = p.dir === 'right' ? 1 : -1;
    const parallel = i > 0 && planks[i - 1].dir === p.dir;
    rots.push(parallel ? rots[i - 1] : r1(-s * mag));
  });
  // yaw about the post (a CSS 3D rotation): each plank swings toward or away from the viewer, within 30 degrees
  const YAWS = [22, -28, 30, -16, 26, -30, 18];
  ...
  const plankGeom = planks.map((p, i) => {
    ...
    bands.push(band((x) => y1 + bowAt(x) + EY - 3, rots[i], postX, mid, 11));
    return {
      p,
      s,
      y0,
  ...
  return (
    ...
    >
      ...
      <g clipPath={`url(#${cid})`}>
        {bands.map((b, k) => (
          <polygon key={k} points={pts2s(b)} fill={INK} stroke="none" />
        ))}
      </g>
      ...
      {plankGeom.map((g, i) => {
        ...
        <a
          ...
          style={style}
        >
          <svg viewBox={`${bx0} ${by0} ${bw} ${bh}`} aria-hidden="true">
            <g>
              {/* thickness faces */}
              {g.sides.map((poly, k) => (
                <polygon
