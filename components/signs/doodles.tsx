...
function Star({ seed, delay = 0, s = 1 }: { seed: number; delay?: number; s?: number }) {
  ...
  return (
    <motion.svg viewBox="0 0 48 48" animate={{ scale: [1, 1.25, 1], rotate: [0, 14, 0] }} transition={{ duration: 3.2, repeat: Infinity, delay, ease: 'easeInOut' }}>
      <Rough kind="poly" points={pts} seed={seed} opts={line(2.2, seed)} />
    </motion.svg>
  );
}
...
function Spiral({ seed }: { seed: number }) {
  ...
  return (
    <svg viewBox="0 0 80 80" className="dd-draw">
      <Rough kind="path" d={d} seed={seed} opts={line(2.2, seed)} />
    </svg>
  );
}
...
function Cloud({ seed }: { seed: number }) {
  return (
    <motion.svg viewBox="0 0 90 60" animate={{ x: [0, 22, 0] }} transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }}>
      <Rough kind="path" d="M 16 40 c -14 0 -18 -18 -4 -22 c 2 -14 22 -16 28 -6 c 10 -8 28 0 24 12 c 12 2 10 18 -4 18 z" seed={seed} opts={{ ...line(2.2, seed), fill: '#fff', fillStyle: 'solid' }} />
...
function Sun({ seed }: { seed: number }) {
  return (
    <svg viewBox="0 0 100 100">
      ...
      <Rough kind="circle" cx={50} cy={50} d={46} seed={seed} opts={{ ...line(2.4, seed), fill: '#fff', fillStyle: 'solid' }} />
...
function Squiggle({ seed }: { seed: number }) {
  return (
    <svg viewBox="0 0 110 40" className="dd-draw">
      <Rough kind="path" d="M 8 20 c 10 -22 20 22 30 0 s 20 -22 30 0 s 20 22 30 0" seed={seed} opts={line(2.2, seed)} />
    </svg>
  );
}
...
function Heart({ seed }: { seed: number }) {
  return (
    <motion.svg viewBox="0 0 50 50" animate={{ scale: [1, 1.18, 1] }} transition={{ duration: 1.4, repeat: Infinity, repeatDelay: 2.6 }}>
      <Rough kind="path" d="M 25 42 c -18 -13 -20 -32 -5 -32 c 4 0 5 4 5 6 c 0 -2 1 -6 5 -6 c 15 0 13 19 -5 32 z" seed={seed} opts={line(2.2, seed)} />
    </motion.svg>
  );
}
...
function Arrow({ seed }: { seed: number }) {
  return (
    <svg viewBox="0 0 110 50" className="dd-draw">
      <Rough kind="path" d="M 100 40 q -40 -34 -90 -10" seed={seed} opts={line(2.2, seed)} />
      <Rough kind="path" d="M 24 16 L 10 30 L 28 34" seed={seed + 1} opts={line(2.2, seed + 1)} />
    </svg>
  );
}
  ...
  return (
    <div className="sg-doodles" aria-hidden="true">
      ...
      <motion.div
        ...
        transition={{ duration: 9, times: [0, 0.25, 0.5, 0.75, 1], repeat: Infinity, repeatDelay: 8, ease: 'easeInOut', delay: 2.5 }}
      >
        <Plane seed={seed + 3} />
      </motion.div>
